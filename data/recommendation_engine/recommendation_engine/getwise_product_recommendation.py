...
class FetchingData():
    def building_connections(self, ):
        ...
        ssh_host = '3.220.181.41'
        ssh_port = 22741
        ssh_user = 'ec2-user'
        ssh_password = 'QZktqgYAQF7aXq8Vk81V'

        # MongoDB connection details
        # mongo_user = 'wiseraidb'
        # mongo_password = 'B5cN5A19T'
        # mongo_database = 'wiserai'  # Replace with your actual database name
        # mongo_collection = 'wiser_evmai_clicks'  # Replace with your actual collection name

        mongo_user = "mongo-admin"
        mongo_password = "VRuAd2Nvmp4ELHh5"
        mongo_database = 'wiserai'  # Replace with your actual database name
        mongo_collection = 'wiser_evmai_clicks'  # Replace with your actual collection name
        ...
        uri = f"mongodb://{mongo_user}:{mongo_password}@localhost:{tunnel.local_bind_port}/{mongo_database}?authSource=admin&retryWrites=true&w=majority"
    ...
    def fetching_unique_shops_db(self, client):
        db = client["wiserai"]
        collection = db["wiser_evmai_clicks"]
        query = {
            '$and': [
                {'product_id': {'$ne': None}},
    ...
    def fetching_data(self, client, shop_name):
        ...
        limit = 10000
        # print(filter_val)
        result = client['wiserai']['wiser_evmai_clicks'].find(
            filter=filter_val,
            limit=limit,
            projection=project
        )
    ...
    def fetching_data_unique(self, client, shop_name, start_date, end_date):
        ...
        result = client['wiserai']['wiser_evmai_clicks'].aggregate(pipeline)
    ...
    def fetch_max_dates(self, client):
        ...
        max_dates = client['wiserai']['wiser_evmai_clicks'].aggregate(max_date_pipeline)
        return max_dates
    ...
    def get_product_info(self, client, shop_address, product_id):
        ...
        projection = {"product_id": 1, "product_handel": 1, "product_type": 1, "product_title": 1, "product_url": 1,
                      "product_image": 1, "product_price": 1,
                      "_id": 0}
        # result = client['wiserai']['wiser_evmai_clicks'].find_one(filter=query,
        #                                                           limit=1,
        #                                                           projection=projection)
        result = client['wiserai']['wiser_evmai_clicks'].find_one(
            filter=query,
            projection=projection,
